...
st.sidebar.markdown("---")
st.sidebar.subheader("🔍 مقارنة شاملة لكل قيم Dead Zone")
dz_max = st.sidebar.slider("أقصى قيمة Dead Zone للمقارنة %", 0.5, 3.0, 1.5, step=0.1)
dz_step = st.sidebar.selectbox("الخطوة بين كل قيمة وإلي بعدها %", [0.05, 0.1, 0.2, 0.25], index=1)
n_values_preview = int(dz_max / dz_step) + 1
st.sidebar.caption(f"هيجرب **{n_values_preview} قيمة** بالظبط (من 0% لـ {dz_max}%، كل {dz_step}%).")
compare_btn = st.sidebar.button("🔍 قارن كل القيم دي تلقائياً", use_container_width=True)
st.sidebar.caption("⚠️ كل ما زاد عدد القيم، كل ما استهلك وقت وموارد أكتر — استخدمها بحكمة.")
...
if compare_btn:
    ...
    for i, dz_pct in enumerate(candidate_values):
        progress.progress(i / len(candidate_values), text=f"جاري تجربة Dead Zone = {dz_pct}% ({i+1}/{len(candidate_values)})...")
        dz = dz_pct / 100.0
        ...
        # موديل أخف (أشجار أقل) وقت المقارنة بس عشان نقدر نجرب قيم كتير بسرعة معقولة
        X_train_cmp = train_df_cmp[feature_cols_cmp]
        y_train_cmp = train_df_cmp["target"]
